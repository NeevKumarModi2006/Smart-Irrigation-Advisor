...
const cropData = {
    barley: { baseWater: 60, waterFrequency: "Every 3 days", baseNPK: [10, 26, 26], fertilizerFrequency: "Monthly" },
    rice: { baseWater: 80, waterFrequency: "Daily", baseNPK: [120, 60, 40], fertilizerFrequency: "Bi-weekly" },
    wheat: { baseWater: 50, waterFrequency: "Every 5 days", baseNPK: [100, 50, 30], fertilizerFrequency: "Monthly" },
    cotton: { baseWater: 70, waterFrequency: "Every 4 days", baseNPK: [80, 40, 40], fertilizerFrequency: "Bi-weekly" },
    maize: { baseWater: 75, waterFrequency: "Every 3 days", baseNPK: [100, 60, 40], fertilizerFrequency: "Bi-weekly" },
    sugarcane: { baseWater: 90, waterFrequency: "Daily", baseNPK: [150, 75, 50], fertilizerFrequency: "Monthly" },
    millet: { baseWater: 55, waterFrequency: "Every 4 days", baseNPK: [80, 40, 30], fertilizerFrequency: "Monthly" },
    soybean: { baseWater: 65, waterFrequency: "Every 3 days", baseNPK: [60, 30, 30], fertilizerFrequency: "Bi-weekly" },
    groundnut: { baseWater: 60, waterFrequency: "Every 4 days", baseNPK: [40, 20, 20], fertilizerFrequency: "Monthly" },
    pulses: { baseWater: 50, waterFrequency: "Every 4 days", baseNPK: [20, 40, 20], fertilizerFrequency: "Monthly" },
    vegetables: { baseWater: 70, waterFrequency: "Daily", baseNPK: [100, 50, 50], fertilizerFrequency: "Bi-weekly" }
};
...
function displayLocationSuggestions(locations) {
    ...
    document.querySelectorAll(".suggestion").forEach((item) => {
        item.addEventListener("click", () => {
            ...
            selectedLocation = locations.find((loc) => loc.display_name === item.textContent);
            suggestionsDiv.innerHTML = "";
        });
    });
}
...
function calculateRecommendations(cropType, soilType, soilHumidity, weatherData) {
    ...
    const adjustedWater = (crop.baseWater * weatherFactor * soilFactor).toFixed(2);
    ...
    return {
        ...
        fertilizer: `NPK ${adjustedNPK.join(':')}, applied ${crop.fertilizerFrequency}`,
        factors: {
...
function generateForecastList(hourlyData) {
    const forecast = [];
    const now = new Date();
    
    // Looking at the next 24 hours of weather
    for (let i = 0; i < 24; i++) {
        ...
        forecast.push(`
            <li>
                ...
                ${temp}°C, ${humidity}% humidity, ${wind} km/h wind
...
getAdviceBtn.addEventListener("click", async () => {
    ...
    const soilHumidity = document.getElementById("soilHumidity").value || "unknown";
        ...
        <div class="advice-section">
            ...
            <div class="advice-list">
                <div class="advice-item">
                    <i class="fas fa-tint" style="color: rgb(0, 0, 255);  translate: 0 5px;"></i> Water: ${recommendations.water}
                </div>
                <div class="advice-item">
                    <i class="fas fa-flask" style="color: rgb(0, 128, 0);  translate: 0 5px;"></i> Fertilizer: ${recommendations.fertilizer}
                </div>
                <div class="advice-item">
                    <i class="fas fa-calculator" style="color: rgb(128, 0, 128); translate: 0 5px;"></i> How we calculated this:
                    <ul>
                        <li>Weather adjustment: ${recommendations.factors.weather}x</li>
                        <li>Soil adjustment: ${recommendations.factors.soil}x</li>
                    </ul>
                </div>
                ${weatherData ? `
                <div class="advice-item">
                    <i class="fas fa-cloud-sun" style="color: rgb(255, 165, 0); translate: 0 5px;"></i> Current Weather:
                    <ul>
                        <li>Temperature: ${weatherData.current.temperature_2m}°C</li>
                        <li>Humidity: ${weatherData.current.relative_humidity_2m}%</li>
                        <li>Wind Speed: ${weatherData.current.wind_speed_10m} km/h</li>
                        ${weatherData.current.precipitation ? `<li>Recent Rainfall: ${weatherData.current.precipitation}mm</li>` : ''}
                    </ul>
                    <div class="weather-forecast">
                        <h4>What's the weather going to be like?</h4>
                        <ul>
                            ${generateForecastList(weatherData.hourly)}
                        </ul>
                    </div>
                </div>
                ` : ''}
